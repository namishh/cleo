...
An AI browser agent that lives in a Chrome side panel. It sees the page through redacted screenshots, reasons with a vision LLM, and drives the browser for you — while PII (names, emails, phones, addresses, IDs) is detected and black-boxed **locally** before anything is sent anywhere.
...
│                                                                 │
│  ┌──────────┐   ┌────────────────────┐   ┌───────────────────┐  │
│  │ SIDE     │   │ SERVICE WORKER     │   │ OFFSCREEN DOC     │  │
│  │ PANEL    │──▶│ (task loop)        │──▶│ (local ML)        │  │
│  │ chat UI  │   │ · orchestration    │   │ · Tesseract OCR   │  │
│  │ steps    │   │ · message routing  │   │ · Kiji PII (ONNX) │  │
│  │ screens  │   │ · per-chat state   │   │ · regex/context   │  │
│  └──────────┘   └─────┬────────┬─────┘   └────────┬──────────┘  │
│                       │        │                  │             │
│                       │        │                  ▼             │
│                       │        │         ┌─────────────────┐    │
...
```
user task (side panel)
      │
      ▼
pin tab via chrome.debugger ──────────────┐
      │                                   │
      ▼                                   │
┌───────────────────────┐                 │
│ OBSERVE               │                 │
...
- **Fail-safe redaction** — model + regex + DOM regions are union-merged; PII never leaves the machine unmasked.
...
- Works on background tabs (CDP input + renderer screenshots)
- Research workflow: open items → remember facts → return to list → summarize top N
- Anti-stall: no-progress detection, scroll-run detection, backend watchdog, auto-resume
...
| Setup | Load unpacked + local Flask backend | Account + cloud | Desktop app install | Build/install a fork | pip/CLI + config |
